$(document).ready(function() {
  if (typeof sankey_datas !== "undefined") {
    for (var i_sankey_index = 0; i_sankey_index < sankey_datas.length; i_sankey_index++) {
      ...
      dragmove = function(link,sankey, width, height) {
        return function(d) {
      ...
      var formatNumber = d3.format(sankey_data.format.number),
          format = function(d) { 
            var n = formatNumber(d);
            n = n.replace(',', '[THOUSAND_REPLACE]').replace('.', mydbr_sorting.separator.decimal).replace('[THOUSAND_REPLACE]', mydbr_sorting.separator.thousand);
            return n + sankey_data.format.suffix; },
      ...
      var node = svg.append("g").selectAll(".node")
          ...
          .call(d3.drag()
            ...
            .on("drag", dragmove(link, sankey, width, height) )
           )
           .on('mouseover', function(d){
             d.targetLinks.forEach(function(elem) {
               d3.select("#link-"+elem.id).classed("highlighted", true);
             });
             d.sourceLinks.forEach(function(elem) {
               d3.select("#link-"+elem.id).classed("highlighted", true);
             });
            })
            .on('mouseleave', function(d){
              d.targetLinks.forEach(function(elem) {
                d3.select("#link-"+elem.id).classed("highlighted", false);
              });
              d.sourceLinks.forEach(function(elem) {
                d3.select("#link-"+elem.id).classed("highlighted", false);
              });

...
//				echo 'Adjusted Formula is ', $value, PHP_EOL;
	...
	public function load($pFilename)
	{
		...
		$zip = new $zipClass;
		$zip->open($pFilename);
    $cont = $this->_getFromZipArchive($zip, "xl/_rels/workbook.xml.rels");
    
    if ($cont===false) {
      throw new PHPExcel_Reader_Exception("MYDBR_IMPORT_INVALID_EXCEL");
    }
		//	Read the theme first, because we need the colour scheme when reading the styles
		$wbRels = simplexml_load_string($this->securityScan($cont), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");

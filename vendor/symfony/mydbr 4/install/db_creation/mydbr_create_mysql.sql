...
order by 
  ...
  end desc, expiration desc, version desc;
END
$$

DROP PROCEDURE IF EXISTS sp_myDBR_license_delete_expired
$$
CREATE PROCEDURE sp_myDBR_license_delete_expired()
BEGIN

delete from mydbr_licenses where expiration<current_date();
...
while(length(vParam_list)>0) do
  ...
  set vParam2 = vParam;
  set v_scale = null;
  if (vParam2 like '%(%,%)%' and vTypeDef in ('decimal', 'double')) then
    set vParam2 = substring( vParam2, locate(',', vParam2)+1);
    set v_scale = substring( vParam2, 1 ,locate(')', vParam2)-1);
    if (v_scale=0) then
      set v_scale = null;
    end if;
  end if;
  ...
  select vParamOrder, substring_index(vParam, ' ', 1), vTypeDef, vLength, v_scale;
...
if (inUseConverted=0) then
  select param_name, type, strlength, fscale
  from ReturnValuesTmp
  order by param_order;
else 
  select t.param_name, t.type, t.strlength, p.title, p.query_name, t.param_order, ifnull(p.optional,0), ifnull(p.only_default,0), p.suffix, p.options, fscale
  from ReturnValuesTmp t 
    left join mydbr_params p on t.param_name = p.param and p.proc_name=vProc_name
...
$$
CREATE PROCEDURE `sp_MyDBR_ProcParams`(inProc_id int, inUseConverted tinyint )
begin
declare vParam_list varchar(2500);
declare vParam varchar(100);
declare vLength int;
...
if (vProc_name is null) then
  select 'No such report' as 'param_name', 'error' as 'type', 0 as 'strlength', 0 as 'fprecision';
else
...
param_name varchar(100),
type varchar(20),
strlength bigint null,
fscale int null
) ENGINE=MEMORY;

set @vProc_name = vProc_name;

PREPARE stmt FROM 
'insert into ReturnValuesTmp
select ORDINAL_POSITION, PARAMETER_NAME, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH, if (NUMERIC_SCALE>0, NUMERIC_SCALE, null)
from INFORMATION_SCHEMA.PARAMETERS 
where SPECIFIC_NAME=? and SPECIFIC_SCHEMA=database()
order by ORDINAL_POSITION';

execute stmt using @vProc_name;
...
if (inUseConverted=0) then
  ...
  select t.param_name, t.type, t.strlength, p.title, p.query_name, t.param_order, ifnull(p.optional,0), ifnull(p.only_default,0), p.suffix, p.options, t.fscale
...
DROP PROCEDURE IF EXISTS `sp_MyDBR_ProcedureParamsTxt`
$$
CREATE PROCEDURE `sp_MyDBR_ProcedureParamsTxt`( inParam_list varchar(2500) )
begin
declare vParam_list varchar(2500);
declare vParam varchar(100);
declare vParam2 varchar(100);
...
$$
CREATE PROCEDURE `sp_MyDBR_ProcedureParams`( inProcName varchar(2500) )
begin
declare vParam_list varchar(2500);
declare vParam varchar(100);
declare vLength int;
...
DROP PROCEDURE IF EXISTS `sp_MyDBR_ReportCategory`
$$
CREATE PROCEDURE `sp_MyDBR_ReportCategory`( vID int, v_category int )
BEGIN
...
DROP PROCEDURE IF EXISTS `sp_MyDBR_FolderCategory`
$$
CREATE PROCEDURE `sp_MyDBR_FolderCategory`( vID int, v_category int )
BEGIN
...
DROP PROCEDURE IF EXISTS `sp_MyDBR_ReportPrivCopy`
$$
CREATE PROCEDURE `sp_MyDBR_ReportPrivCopy`( in_report_id int, in_reportgroup int )
BEGIN

declare v_folder_id int;
declare v_reportgroup int;

select folder_id, reportgroup into v_folder_id, v_reportgroup
from mydbr_reports
where report_id = in_report_id;

if (in_reportgroup = 0 ) then
  set v_reportgroup = null;
end if;
...
and report_id in (
  select report_id
  from mydbr_reports
  where folder_id = v_folder_id and reportgroup = ifnull(v_reportgroup, reportgroup)
);
...
where r.folder_id = v_folder_id  and r.report_id != in_report_id and r.reportgroup = ifnull(v_reportgroup, reportgroup);
...
if (vCnt>0) then
  ...
  if (vCnt=0) then
    insert into mydbr_templates ( name, header, rowdata, footer, folder_id, creation_date )
...
select name, header, rowdata, footer
from mydbr_templates

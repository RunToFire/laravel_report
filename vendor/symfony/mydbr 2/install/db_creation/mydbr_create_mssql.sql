...
go
alter table mydbr_param_queries alter column name nvarchar(255) not null
go
...
go
alter table mydbr_params alter column query_name nvarchar(255) null
go
alter table mydbr_params alter column default_value nvarchar(255) null
go
...
go
alter table mydbr_statistics alter column query nvarchar(max) not null
go
...
alter table mydbr_options alter column value varchar(512) not null
go
...
if object_id('mydbr_sync_exclude', 'U') is not null begin
drop table mydbr_sync_exclude
end
go
...
if object_id('mydbr_scheduled_tasks', 'U') is null begin
...
go
exec sp_MyDBR_OptionInit 'proxy_auth', 's:0:"";'
go
exec sp_MyDBR_OptionInit 'proxy_type', 's:4:"HTTP";'
go
...
order by 
  ...
  end desc, expiration desc
end
go

if object_id('sp_myDBR_license_delete_expired', 'P') is not null
drop procedure sp_myDBR_license_delete_expired
go
CREATE PROCEDURE sp_myDBR_license_delete_expired
as
begin

delete 
from mydbr_licenses 
where expiration<cast(convert(varchar(10), getdate(), 120) as datetime)
...
if object_id('sp_MyDBR_param_in_use', 'P') is not null
...
select distinct r.report_id, p.proc_name, r.name, r.folder_id
from mydbr_params p
...
select substring(c.name,2, 128), t.name, c.length, c.colid, case when t.name in ('decimal', 'numeric') then c.scale else null end
from syscolumns c, systypes t
where object_name(id) = @inProcName and c.xtype=t.xusertype

update #ReturnValuesTmp set datalen=datalen/2
where lower(type) in ('nchar', 'nvarchar') and datalen!=-1

update #ReturnValuesTmp set datalen=2147483647
where lower(type) in ('text', 'ntext')

update #ReturnValuesTmp set datalen=8000
where lower(type) in ('varchar', 'nvarchar') and datalen=-1
...
select param_name, type, datalen, fscale
...
if (@UseConverted=0) begin
  ...
  select t.param_name, t.type, t.datalen, p.title, p.query_name, t.param_order, isnull(p.optional,0), isnull(p.only_default,0), p.suffix, p.options, t.fscale
  from #ReturnValuesTmp t, mydbr_params p 
  where t.param_name = p.param and p.proc_name=@vProc_name
  union
  select t.param_name, t.type, t.datalen, null, null, t.param_order, 0, 0, null, null, t.fscale
...
if object_id('sp_MyDBR_ReportCategory', 'P') is not null
drop procedure sp_MyDBR_ReportCategory
go
CREATE PROCEDURE sp_MyDBR_ReportCategory
@vID int, 
@v_category int
as
begin
...
update mydbr_reports
set reportgroup=@v_category
where report_id = @vID

end
go

if object_id('sp_MyDBR_FolderCategory', 'P') is not null
drop procedure sp_MyDBR_FolderCategory
go
CREATE PROCEDURE sp_MyDBR_FolderCategory
@vID int, 
@v_category int
as
begin
...
if object_id('sp_MyDBR_ReportPrivCopy', 'P') is not null
drop procedure sp_MyDBR_ReportPrivCopy
go
create procedure sp_MyDBR_ReportPrivCopy
@in_report_id int, 
@in_reportgroup int
as
begin
...
declare @v_reportgroup int

select @v_folder_id = folder_id, @v_reportgroup = reportgroup
from mydbr_reports
where report_id = @in_report_id

if (@in_reportgroup = 0 ) begin
  select @v_reportgroup = null
end
...
and report_id in (
  select report_id
  from mydbr_reports
  where folder_id = @v_folder_id and reportgroup = isnull(@v_reportgroup, reportgroup)
)

insert into mydbr_reports_priv (report_id, username, group_id, authentication)
select r.report_id, p.username, p.group_id, p.authentication
from mydbr_reports r
  join mydbr_reports_priv p on p.report_id=@in_report_id
where r.folder_id = @v_folder_id  and r.report_id != @in_report_id and r.reportgroup = isnull(@v_reportgroup, r.reportgroup)
...
if (@vAuth_DB > 0) begin
  ...
  if (@inSearch!='') begin
    insert into #Users_tmp ( username, name, auth_source, haspriv )
    select top(20) u.name, '', @vAuth_DB, 0
    from sysusers u
    where lower(u.name) like '%'+lower(@inSearch)+'%' and not exists (
      select * 
      from mydbr_folders_priv p
...
if (@vAuth_myDBR > 0 or @vAuth_SSO > 0 or @vAuth_LDAP > 0 or @vAuth_Custom > 0) begin
  ...
  if (@inSearch!='') begin
    insert into #Users_tmp ( username, name, auth_source, haspriv )
    select top(20) u.username, u.name, u.authentication, 0
    from mydbr_userlogin u
    where (lower(u.username) like '%'+lower(@inSearch)+'%' or lower(u.name) like '%'+lower(@inSearch)+'%') and not exists (
      select * 
      from mydbr_folders_priv p
...
if (@in_sp_single is not null) begin
  ...
  if (@in_source_folder is not null) begin
    ...
    where 
      ...
      r.ROUTINE_CATALOG = DB_NAME() and 
      r.ROUTINE_NAME not like 'sp_MyDBR%' and
      r.ROUTINE_NAME not like 'sp_DBR_demo_%' and
      r.ROUTINE_NAME not in ('sp_DBR_StatisticsReport', 'sp_DBR_StatisticsSummary', 'fn_mydbr_column_exists', 'fn_BegOfDay', 'fn_EndOfDay', 'mydbr_style')
...
if (@v_report_group_id is null) begin
  ...
  select @v_report_group_id = scope_identity()
end
...
if object_id('sp_MyDBR_sync_report_path_get', 'P') is not null
...
declare @v_path varchar(max)
declare @v_path2 varchar(max)
declare @v_folder varchar(max)
...
while( @v_pos > 0 and @v_incorrect = 0) begin
  ...
  select @v_path2 = substring( @v_path, @v_pos+len(@in_delim), 32768 )
...
if object_id('sp_MyDBR_sync_report_path_set', 'P') is not null
...
if object_id('sp_MyDBR_sync_report_priv_get', 'P') is not null
...
if object_id('sp_MyDBR_sync_report_priv_rst', 'P') is not null
...
if object_id('sp_MyDBR_sync_check_priv', 'P') is not null
...
if object_id('sp_MyDBR_sync_report_priv_set', 'P') is not null
...
if object_id('sp_MyDBR_sync_reportgrp_check', 'P') is not null
...
if (@in_report_group_id is null) begin
  ...
  select @in_report_group_id = scope_identity()
end

end
go

if object_id('sp_MyDBR_sync_folder_info', 'P') is not null
...
if object_id('sp_MyDBR_sync_folder_priv_get', 'P') is not null
...
if object_id('sp_MyDBR_sync_folder_priv_rst', 'P') is not null
...
if object_id('sp_MyDBR_sync_folder_priv_set', 'P') is not null
...
if object_id('sp_MyDBR_sync_drop_sync_folder', 'P') is not null
...
if object_id('sp_MyDBR_scheduled_tasks', 'P') is not null
...
if object_id('sp_MyDBR_scheduled_task_set', 'P') is not null
...
if object_id('sp_MyDBR_scheduled_task_del', 'P') is not null
...
if object_id('sp_MyDBR_scheduled_update_run', 'P') is not null
...
if object_id('sp_MyDBR_report_location', 'P') is not null
...
if object_id('sp_MyDBR_check_mydbr_username', 'P') is not null
...
if object_id('sp_MyDBR_scheduler_users', 'P') is not null
...
go
insert into mydbr_version values ('5.1.1')
go
if object_id('fn_seconds_to_time_str', 'FN') is not null
drop function fn_seconds_to_time_str
go
...
select top(@inRowCount) 'dbr.rownum' as '#', isnull(u.name , s.username) as 'Name', count(*) as 'RunCount', s.username
from mydbr_statistics s 
...
select top(@inRowCount) 'dbr.rownum' as '#', r.name as 'Report', s.proc_name as 'StoredProcedure', count(*) as 'RunCount', r.report_id as 'ReportID'
from mydbr_statistics s

...
if object_id('sp_myDBR_license_delete_expired') is not null
...
delete from mydbr_licenses where expiration<cast(getdate() as date)
...
select substring(c.name,2, 128), t.name, c.length, c.colid, c.scale
...
select substring(p.parmname,2,128), isnull(p.user_type, p.parmdomain), p.length, p.parm_id, null
from SYSPROCPARMS p
...
if (@UseConverted=0) begin
  ...
  select t.param_name, t.type, t.datalen, p.title, p.query_name, t.param_order, isnull(p.optional,0), isnull(p.only_default,0), p.suffix, p.optionss, t.fscale
...
if object_id('sp_MyDBR_ReportCategory') is not null
...
if object_id('sp_MyDBR_FolderCategory') is not null
...
if object_id('sp_MyDBR_ReportPrivCopy') is not null
